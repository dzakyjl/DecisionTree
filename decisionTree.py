...
#       self.impurity_score = self.__calcuate_impurity_score(self.data[self.data])
...
#     left_rows = self.data[self.data[self.split_feature] <= self.criterial]
#     right_rows = self.data[self.data[self.split_feature] > self.criterial]
#     self.left.fit(data = left_rows, target = self.target)
#     self.right.fit(data = right_rows, target = self.target)
    
#   def __calcuate_impurity_score(self, data):
...
#       score = self.__calculate_information_gain(left_count = len(left_data), left_impurity = left_impurity, right_count = len(right_data), right_impurity = right_impurity)
...
#     non_numeric_columns = self.data[self.independent].select.dtypes(include=['category', 'object', 'bool']).columns.tolist()
